...
def prepare_query(query, uid, cover_entities=True):
    ...
    predicates = [
        predicate.replace(',\n', '')
        for predicate in open(str(ROOT_PATH) + '/data/paraqa/predicates.txt').readlines()
    ]
...
def get_all_verbalized_answers(example):
    return [example['verbalized_answer'], example['verbalized_answer_2'], example['verbalized_answer_3'], example['verbalized_answer_4'],
            example['verbalized_answer_5'], example['verbalized_answer_6'], example['verbalized_answer_7'], example['verbalized_answer_8']]

def preprocess(data):
    for d in tqdm.tqdm(data):
        uid = d['uid']
        question = d['question']
        answers = get_all_verbalized_answers(d)
        query = d['query']

        # cover entities
        covered_entities_question = cover_entities(question)
        covered_entities_answers = [cover_entities(answer, is_answer=True) for answer in answers if answer]
        ...
        d[PREPROCESSED] = {
            ...
            ANSWER: covered_entities_answers,
...
def write_data(data, name):
    with open(f'{str(ROOT_PATH)}/data/paraqa/preprocessed_{name}.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)

if __name__ == "__main__":
    templates = { temp['id']: temp for temp in json.load(open(str(ROOT_PATH) + '/data/paraqa/templates.json')) }
    sparql_templates_ids = read_sparql_template_ids()

    train_data = json.load(open(f'{str(ROOT_PATH)}/data/paraqa/train.json'))
    test_data = json.load(open(f'{str(ROOT_PATH)}/data/paraqa/test.json'))
